...
export default function Formations({language}: Props) {
    ...
    const LoadFormations = (): any => {
        ...
        if(language?.code == "pt-PT") {
            ...
            return formationsArrayPT.map((el, n) => {
                ...
                return <Formation key={n} actualText={el?.actual} code={language?.code} curso={el?.curso} escola={el?.escola} image={el?.image} dataInicio={el?.dataInicio} dataFim={el?.dataFim || undefined} 
                skills={el?.skills}/>
            ...
            return formationsArrayEN.map((el, n) => {
                ...
                return <Formation actualText={el?.actual} key={n} code={language?.code} curso={el?.curso} escola={el?.escola} image={el?.image} dataInicio={el?.dataInicio} dataFim={el?.dataFim || undefined} 
                skills={el?.skills}/>
    ...
    return (
        <section id='formation' className='relative z-10'>
            ...
            <div className='relative w-[90%] m-auto 3xl:w-[70%] flex flex-col justify-between p-4 z-0'>
                <h1 className='text-blue-900 text-lg italic'>{"<"}{language?.formation?.section}{">"}</h1>
